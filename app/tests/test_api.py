...
class TestEventsAPI:
    ...
    def test_service_instance_event_ok(self, test_client):
        ...
        response = test_client.post(
            ...
            json={"instance_id": service_instance.id, "updated_status": ServiceInstanceStatus.CRASH_LOOP},
        )
        assert response.status_code == 200
    ...
    def test_service_instance_event_validation(self, test_client):
        ...
        response = test_client.post(
            "/api/events/service-instances/",
            json={"instance_id": service_instance.id, "updated_status": ServiceInstanceStatus.EVICTED},
        )
        assert response.status_code == 422
...
def _serialize_service_model(service_model, **kwargs):
    return {
        ...
        "resource_limit": {"cpu_cores": service_model.cpu_cores, "disk": service_model.disk, "ram": service_model.ram},
        "instance_id": None,
    } | kwargs
